...
module.exports =class FeedBack {
    findAll = async () => {
     ...
     findOne = async (id) => {
        try {
            ...
            if (Object.keys(rs).length == 0) {
                return Promise.reject({ messager: " FeedBack not exists ! "  });
            }
            if (rs) {
    ...
    findIdAnime = async (item) => {
        try {
        const rs = await Repository.findIdAnime(item);
        if (Object.keys(rs).length == 0) {
            return Promise.reject({messager :"Not Found findIdAnime"} )
        }
        return Promise.resolve({result : rs})
    } catch (error) {
        return Promise.reject({messager :"Not Found findIdAnime"})
